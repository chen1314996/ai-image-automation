...
const BASE_URL = process.env.TEST_BASE_URL || 'http://localhost:3066';
const TEST_DATA_DIR = path.join(__dirname, '..', 'data');
const TEST_INPUT_DIR = path.join(TEST_DATA_DIR, 'input');
const TEST_LEGIL_REF_DIR = path.join(TEST_DATA_DIR, 'legil-ref');
const results = [];
...
async function runTests() {
    ...
    try {
        const res = await request('/api/count-images', {
            ...
            body: { folderPath: TEST_INPUT_DIR }
        });
    ...
    try {
        const res = await request('/api/config/legil-ref-folder', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: { folderPath: TEST_LEGIL_REF_DIR }
        });
