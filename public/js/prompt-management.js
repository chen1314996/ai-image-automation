// 提示词管理面板：获取、复制、选择并发送提示词到生图平台。
        ...
        function createPromptCard(prompt, number) {
            ...
            card.innerHTML = `
                ...
                <div class="prompt-actions">
                    ...
                    <button class="btn-small btn-send" onclick="sendPromptToLegil(${number})" id="btn-send-${number}" ${isSent ? 'disabled' : ''} title="发送到生图平台生成图片">
                        发送到生图平台
                    </button>
                </div>
            `;
        ...
        async function sendPromptToLegil(number) {
            ...
            addLog(`正在发送提示词 ${number} 到生图平台...`, 'system');
            ...
            try {
                ...
                if (data.success) {
                    ...
                    addLog(`提示词 ${number} 已发送到生图平台`, 'success');
                } else {
        ...
        async function sendAllPromptsToLegil() {
            ...
            addLog(`开始批量发送 ${currentPrompts.length} 组提示词到生图平台...`, 'system');
        ...
        function onWorkflowComplete() {
            if (!document.getElementById('promptsSection')) return;
            setTimeout(() => {
                fetchLatestPrompts();
            }, 1000);
        }
