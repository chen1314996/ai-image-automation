...
根据创意知识库自动选方向
Agent 生成新方向和 prompt
Legil Nano Banana 2 / 1:1 / 2K / 每 prompt 4 张
```
...
creative-expansion-agent/
  ...
  PROJECT_SETTINGS.md                # 当前项目运行设定；后续用户调整时必须同步
...
2. `PROJECT_SETTINGS.md`
   当前项目设定，写清楚 Legil 参数、目录、每日上限、失败暂停、通知策略和同步规则。后续你调整需求时，先改这里，再检查主提示词和相关 skill。
3. `instructions.md`
...
如果你想让这些 skill 变成 Codex 环境可自动触发的 skill，可以把 `skills` 下的 6 个目录复制到：
...
2. 同时读取 `PROJECT_SETTINGS.md`，拼到 system/developer prompt 中。
3. 把用户输入、表格提取文本、图片 OCR 结果、TOP 素材洞察、参考图描述放进 user message。
4. 要求模型按默认四部分表格输出。
...
3. 根据输入类型加载对应 skill：
   - 默认加载前三个核心 skill 和 `legil-run-once-prompt-contract`。
...
用户上传参考图/表格/方向文本
  ...
  -> 返回 Markdown 表格
  -> 前端展示或后端入库
  -> run-once 场景下将“新方向拓展表”解析成 prompts 数组并发送 Legil
```
...
- `projectSettings`：当前 Legil 参数、运行上限、输出目录、通知策略。

如果图片还没有被模型识别，项目至少要先给出每张图的编号和基础描述，避免 Agent 在没有视觉信息的情况下假装看过图。

## 输出建议

默认输出 Markdown 表格，适合人工审阅和复制到表格工具。后台 `run-once` 场景优先只需要 `新方向拓展表`，列名必须稳定，方便程序解析。正式产品里建议再增加一个可选参数：
...
只要你后续调整模型、比例、分辨率、每 prompt 张数、每日上限、输出目录、参考图目录、飞书通知、禁用元素或方向表字段，优先更新：

```text
PROJECT_SETTINGS.md
instructions.md
agent.yaml
skills/legil-run-once-prompt-contract/SKILL.md
```

如果调整影响参考分析、TOP 素材判断、表格逐行处理或批量策略，再同步对应 skill。
